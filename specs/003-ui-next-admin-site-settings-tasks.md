...
- [x] 页面内按分组展示基础信息、品牌资源、SEO、首页运营位
- [x] 表单提交成功后提供统一成功提示
- [x] 表单校验失败时提供统一错误表现
- [x] 确认“基础信息”和“SEO 元信息”不是同一层语义，避免后续把 SEO title 继续塞进 general
...
- [x] 管理员可在 `ui-next` 完成 `general/branding/seo` 配置修改
- [x] 管理员可在 `ui-next` 完成两个首页运营位配置修改
- [x] 刷新网站设置页后能正确回显后端已保存数据

## Phase D：前台消费真实站点配置

目标：移除首页运营位和公共品牌信息对硬编码内容的依赖，改为消费真实后台配置。
...
- [x] 新增公开站点信息读取 API 封装
- [x] 浏览器标题与 favicon 消费后台基础信息和品牌配置
- [x] 修正 `ui-next/index.html` 的静态默认 title / description，占位值不再沿用 Figma 文案
...
- [x] 浏览器标题和 favicon 已不再依赖硬编码站点品牌信息
- [x] 查看网页源代码时，`ui-next` 静态入口页已存在可接受的默认 title / description
...
- [ ] 验证 `general/branding/seo` 保存成功
- [ ] 验证 `homepage` 两个运营位保存成功
- [ ] 验证首页 Banner 实时读取真实配置
- [ ] 验证热门文章广告位关闭时直接隐藏
- [ ] 验证站点名称、favicon 修改后前台即时读取真实配置
- [ ] 验证左侧导航切换不同后台页时状态正确
