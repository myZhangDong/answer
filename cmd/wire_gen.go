...
import (
	...
	"github.com/apache/answer/internal/service/project_common"
	...
	"github.com/apache/answer/internal/service/video_common"
...
func initApplication(debug bool, serverConf *conf.Server, dbConf *data.Database, cacheConf *data.CacheConf, i18nConf *translator.I18n, swaggerConf *router.SwaggerConfig, serviceConf *service_config.ServiceConfig, uiConf *server.UI, logConf log.Logger) (*pacman.Application, func(), error) {
	...
	contentReviewService := content_review.NewContentReviewService(serviceConf)
	commentController := controller.NewCommentController(commentService, rankService, captchaService, rateLimitMiddleware, contentReviewService)
	...
	questionController := controller.NewQuestionController(questionService, answerService, rankService, siteInfoCommonService, captchaService, rateLimitMiddleware, contentReviewService)
	answerController := controller.NewAnswerController(answerService, rankService, captchaService, siteInfoCommonService, rateLimitMiddleware, contentReviewService)
	...
	videoRepo := video.NewVideoRepo(dataData, uniqueIDRepo)
	videoCommon := videocommon.NewVideoCommon(videoRepo)
	videoController := controller.NewVideoController(videoCommon)
