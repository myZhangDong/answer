...
func (a *UIRouter) Register(r *gin.Engine, baseURLPath string) {
	staticPath := os.Getenv(constant.AnswerStaticPathEnv)
	...
	if staticPath != "" {
		...
		if err != nil || !info.IsDir() {
			...
			staticDir := filepath.Join(staticPath, "static")
			if info, err := os.Stat(staticDir); err == nil && info.IsDir() {
				r.Static(baseURLPath+"/static", staticDir)
			}
			assetsDir := filepath.Join(staticPath, "assets")
			if info, err := os.Stat(assetsDir); err == nil && info.IsDir() {
				r.Static(baseURLPath+"/assets", assetsDir)
			}
			r.NoRoute(func(c *gin.Context) {
				...
				filePath, ok := resolveStaticFile(staticPath, urlPath)
				if ok {
					c.File(filePath)
					return
				}
				// Avoid serving index.html for missing asset requests.
				if pathLooksLikeAsset(urlPath) {
...
func resolveStaticFile(staticPath, requestPath string) (string, bool) {
	cleanPath := strings.TrimSpace(requestPath)
	if cleanPath == "" || cleanPath == "/" {
		return "", false
	}
	cleanPath = strings.TrimPrefix(cleanPath, "/")
	fullPath := filepath.Join(staticPath, filepath.FromSlash(cleanPath))
	relativePath, err := filepath.Rel(staticPath, fullPath)
...
func pathLooksLikeAsset(requestPath string) bool {
	cleanPath := strings.TrimSpace(requestPath)
	if cleanPath == "" || cleanPath == "/" {
		return false
	}
